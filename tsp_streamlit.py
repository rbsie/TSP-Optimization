...
def run_tsp_model_mtz(cities_df):
    """
...
if run_opt:
    ...
    if solver_method == 'MTZ (Miller-Tucker-Zemlin)':
        with st.spinner(f'Calculating optimal tour for {num_cities} cities using MTZ... (This may take time)'):
            st.session_state.results = run_tsp_model_mtz(cities_to_solve)
        ...
        st.warning(f"The selected solver **({solver_method})** is not implemented. \n\nPlease select **MTZ (Miller-Tucker-Zemlin)** from the sidebar and click 'Run Optimization' again.")
...
if st.session_state.results is not None:
    ...
    if r['status'] == 'optimal':
        ...
        col1.metric(label="Solver Status", value=str(r["status"]).capitalize())
        col2.metric(label="Optimal Distance (Tour)", value=f"{r['obj']:.2f} km")
        ...
        st.error(f"No optimal solution found. Solver-Status: {r['status']}")
